...
Whatever you do, **always install from `wattpost.cloud/app`**, never from the per-appliance URL (`humnb7h4n6.wattpost.cloud/` etc). The reason's [below](#why-not-the-broker-url).
...
Each appliance has its own remote-access URL, `humnb7h4n6.wattpost.cloud/`, `5xkpr8wxy3.wattpost.cloud/`, etc. These open the appliance's local dashboard through a Cloudflare tunnel.

You **can** browse them, but you should not **install** them as a PWA.

Push notifications register against the page's origin (the part of the URL before the first `/`). If you install a PWA from `humnb7h4n6.wattpost.cloud/`:

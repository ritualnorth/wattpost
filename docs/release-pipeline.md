...
- Binaries are served from `releases.wattpost.io`, fronted by Cloudflare with a 24h cache (safe because filenames are versioned). Directory browsing is enabled so the listing is human-readable.
- `latest.img.xz` and `latest.tar.gz` are symlinks, repointed to the newest build on each publish.
...
`releases.wattpost.io` as the final step. That SSH publish
...
**scp step fails.** The build runs as a GH Actions hosted runner, talking to the release host over SSH with the `VPS_SSH_KEY` secret. If scp 502s or auth-fails, regenerate the key on the host and update the GH secret.
...
2. CF dashboard → DNS. Record should be `A` / `releases` / the origin IP / Proxied.
3. `curl -sI --resolve releases.wattpost.io:443:"$ORIGIN_IP" https://releases.wattpost.io/ -k`. Should 200 from the origin host. If yes, the break is CF→origin; if no, Caddy isn't serving it.
4. Caddy block syntax: `docker exec <caddy-container> caddy validate --config /etc/caddy/Caddyfile`.
